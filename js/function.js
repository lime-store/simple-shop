$(document).ready(function() {
    ...
    $(document).on('click', '.open-add-to-card-modal', function() {
        ...
        let optionList = $this.find('.product-option-list').html();
        ...
        $('.modal-product-option-list').html(optionList);
    ...
    $(document).on('click', '.add-to-card', function() {
        ...
        $this.find('.opt-value.active').each(function() {
          let name = $(this).closest('.opt-list-wrp').find('.opt-name').text();
          let value = $(this).text();
    ...
    $(document).on('click', '.send-cart', function() {
      ...
      Object.keys(card).map(function(objectKey, index) {
        ...
        strs = strs + `${orderViewSrc}${orderId}`;
        ...
        $.ajax({
          ...
          success: () => {
            ...
            let url = `https://wa.me/${phoneNumber}?text=${encodedStrs}`;
  ...
  $(document).on('click', '.share', function() {
    ...
    let url = `${shareSrc}${dataName}`;
    let fixedUrl = url.replace(/ /g, '20/'); 
    let whatsappLink = `https://wa.me/${phoneNumber}?text=${encodeURIComponent(fixedUrl)}`;
  ...
  function prepareProductCardTpl(product) {
    ...
    if(product.addedDate) {
      ...
      if(Array.isArray(product.imageSrc)) {
        product.imageSrc.forEach(val => {
          imageContent += `<div class="swiper-slide prodcuts-image"><img src="${val}"></div>`;
        });
      } else {
        imageContent = `<div class="swiper-slide prodcuts-image"><img src="${product.imageSrc}"></div>`;
      }
...
return ` <div class="products-card animate__animated animate__fadeIn">
          ...
          <div class="product-price-container">
            ...
            ? `<div class="product-discount-price">
                  <span class="products-price">${(product.price - ((product.price) * product.discount / 100)).toFixed(2)}₼</span>
                  <span style="font-size: 11px; margin-left: 5px;">(-${product.discount}%)</span>
               </div>` 
            : `<span class="products-price">${product.price}₼</span>` 
            }
          </div>
                  
          <button class="button open-add-to-card-modal"><i class="las la-cart-arrow-down"></i> Səbətə</button>
  ...
  function appendProductCard(product) {
    return new Promise((resolve) => {
      const tpl = prepareProductCardTpl(product);
      const $card = $(tpl);
      $('.products-list').append($card);

      requestAnimationFrame(() => {
        const swiperEl = $card.find('.productImageSlider')[0];
        if (swiperEl) {
          new Swiper(swiperEl, {
            grabCursor: true,
            loop: false, // loop: true требует минимум 3 слайда, ставь false
            spaceBetween: 10,
